...
7. Check **only** `## Step status` item **a)** when done — do not edit other plan content.
8. On incomplete/invalid Descriptor (including stripping template errors, adding forbidden dedicated codes on new routes, or single-use components): **`fail`**, do not check the step.
