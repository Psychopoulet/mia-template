...
Conclude: 5 lines per [reference.md](../reference.md). Deliverables: Mediator/Server/`utils`/`@types`; autonomous helpers as files not `private` methods; no param validation; no host-auth reimplementation; comments ≥ 25 lines. Checks: `lint-back`, `build-back`. Next: `mia-git` commit then `mia-tests`.
